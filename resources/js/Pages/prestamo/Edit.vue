...
watchEffect(() => {
    if (props.show) {
        ...
        //     form[element] =  props.prestamoa[element]
        // });
        form.errors = {}
        props.titulos.forEach(names => {
            form[names['order']] = props.prestamoa[names['order']]
        });

        // form.codigo = props.prestamoa?.codigo
    }
})

const update = () => {
    form.put(route('prestamo.update', props.prestamoa?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
                    ...
                    <div v-for="(atributosform, indice) in data.printForm" :key="indice">
                        <div v-if="atributosform.type == 'id'" id="SelectVue">
                            ...
                            <v-select :options="data[atributosform.idd]" label="title" v-model="form[atributosform.idd]"
                                :value="data[atributosform.idd][props.prestamoa.prestamo_id]"></v-select>
